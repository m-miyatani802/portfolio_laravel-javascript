...
@section('content')
    <div class="row-my-2">
        ...
        <div class="col-sm-8">
            <h1>お気に入り</h1>
            <h2>~単語一覧~</h2>
            @if (count($items['favorites_words']) > 0)
                <a href="{{ route('typ.favorite') }}">タイピング練習</a>
                <table class="table table-striped table-hover">
                    <tr>
                        <th><a href="{{ route('favorite.words_index.sort') }}?sort=created_at">最新</a></th>
                        <th><a href="{{ route('favorite.words_index.sort') }}?sort=reading">読み方</a></th>
                        <th><a href="{{ route('favorite.words_index.sort') }}?sort=phrases">単語</a></th>
                        <th><a href="{{ route('favorite.words_index.sort') }}?sort=meaning">意味</a></th>
                    </tr>
                    @foreach ($items['favorites_words'] as $item)
                        @if (!empty($item->reading))
                            <tr>
                                <td>　　　</td>
                ...
                @elseif(!empty($sort))
                    {{ $items['favorites_words']->appends(['sort' => $sort])->links() }}
                @endif
            @else
                <p>お気に入りはありません</p>
            @endif
