    ...
    <div class="row-my-2">
        ...
        <div class="col-sm-8">
            ...
            @if (count($items['words']) > 0)
                <a href="{{ route('typ.index') }}">タイピング練習</a>
                <table class="table table-striped table-hover">
                    <tr>
                        <th><a href="{{ route('index.sort') }}?sort=created_at">最新</a></th>
                        <th><a href="{{ route('index.sort') }}?sort=reading">読み方</a></th>
                        <th><a href="{{ route('index.sort') }}?sort=phrases">単語</a></th>
                        <th><a href="{{ route('index.sort') }}?sort=meaning">意味</a></th>
                        <th>登録者</th>
                    </tr>
                        ...
                        <tr>
                            ...
                            <td class="align-middle button">
                                <span class="btn-group">
                                    <form action="{{ route('words.show', $item->id) }}" method="get">
                                        @csrf
                                        {{-- <input type="hidden" name="id" value="{{ $item->word_id }}"> --}}
            ...
            @else
                ...
                <form action="{{ route('words.create') }}" method="get">
                    <input type="submit" class="btn btn-primary mx-sm-1" value="単語登録">
                </form>
            @endif
        </div>
        <div class="col-sm-2"></div>
